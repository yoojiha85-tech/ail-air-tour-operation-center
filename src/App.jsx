...
export default function App(){
  ...
  const TODAY_WORK_LABEL={customer_balance:'고객 잔금',final_check:'최종체크',passport_copy:'여권사본',intermediate_air:'중간항공',intermediate_air_notice:'중간항공',reconfirm:'항공·호텔 재확인',briefing:'설명회',travel_docs:'여행서류',departure_notice:'출발안내',fx_notice:'환율안내',land_work:'랜드사 업무'}
  const todayWorkSummary=useMemo(()=>({
    ...
    reconfirm:todayWorkCenter.filter(x=>x.task_type==='reconfirm').length,
    briefing:todayWorkCenter.filter(x=>x.task_type==='briefing').length,
    docs:todayWorkCenter.filter(x=>x.task_type==='travel_docs').length,
    departure:todayWorkCenter.filter(x=>x.task_type==='departure_notice').length
  }),[todayWorkCenter])
  const visibleTodayWork=useMemo(()=>{
    ...
    else if(todayWorkFilter==='intermediate_air_notice')list=list.filter(x=>['intermediate_air','intermediate_air_notice'].includes(x.task_type))
...
async function goToTodayWork(x){
    ...
    if(['final_check','passport_copy','intermediate_air','intermediate_air_notice','reconfirm','briefing','travel_docs','departure_notice','fx_notice'].includes(x.task_type)){
  ...
  function todayQuickActions(x){
    ...
    if(['intermediate_air','intermediate_air_notice'].includes(x.task_type))return <div className="todayQuickActions">{!r.intermediate_air_deposit_paid&&<button className="quickPrimary" disabled={!has(member,'reservation_edit')} onClick={()=>quickReservationUpdate(r.id,{intermediate_air_deposit_paid:true,intermediate_air_deposit_paid_at:new Date().toISOString().slice(0,10)},'중간항공 중도금 결제를 확인 처리했습니다.')}>중도금 결제 확인</button>}{!r.intermediate_air_nonrefundable_notice_done&&<button className="quickSecondary" disabled={!has(member,'reservation_edit')} onClick={()=>quickReservationUpdate(r.id,{intermediate_air_nonrefundable_notice_done:true,intermediate_air_nonrefundable_notice_at:new Date().toISOString().slice(0,10)},'중간항공 환불불가 안내를 완료 처리했습니다.')}>환불불가 안내 완료</button>}</div>
    if(x.task_type==='reconfirm')return <div className="todayQuickActions"><button className="quickPrimary" disabled={!has(member,'reservation_edit')} onClick={()=>quickReservationUpdate(r.id,{air_recheck_done:true,supplier_confirmation_received:true},'항공·호텔 재확인을 완료 처리했습니다.')}>재확인 완료</button></div>
    if(x.task_type==='briefing')return <div className="todayQuickActions"><button className="quickPrimary" disabled={!has(member,'reservation_edit')} onClick={()=>quickReservationUpdate(r.id,{briefing_notice_done:true},'여행 설명회 안내를 완료 처리했습니다.')}>설명회 안내 완료</button></div>
    if(x.task_type==='travel_docs')return <div className="todayQuickActions"><button className="quickPrimary" disabled={!has(member,'reservation_edit')} onClick={()=>quickReservationUpdate(r.id,{travel_docs_delivered:true},'최종 여행서류 전달을 완료 처리했습니다.')}>여행서류 전달 완료</button></div>
    if(x.task_type==='departure_notice')return <div className="todayQuickActions"><button className="quickPrimary" disabled={!has(member,'reservation_edit')} onClick={()=>quickReservationUpdate(r.id,{departure_notice_done:true,departure_notice_at:new Date().toISOString()},'출발 전 최종 안내를 완료 처리했습니다.')}>출발안내 완료</button></div>
    if(x.task_type==='fx_notice')return <div className="todayQuickActions"><button className="quickPrimary" disabled={!has(member,'reservation_edit')} onClick={()=>quickReservationUpdate(r.id,{fx_notice_done:true,fx_notice_at:new Date().toISOString().slice(0,10)},'환율 변동 안내를 완료 처리했습니다.')}>환율안내 완료</button></div>
  ...
  function openRemittance(r){
    ...
    setRemitModal({
      ...
      expense_type:'land',remittance_stage:'application',
      due_date:'',paid_date:'',currency:'USD',foreign_amount:'',exchange_rate:'',amount_krw:'',note:''
    })
  }

  function openRemittanceEdit(e){
    if(!has(member,'expense_manage'))return alert('지출·송금 수정 권한이 없습니다.')
    const r=rows.find(v=>v.id===e.reservation_id)
    if(!r)return
    setRemitModal({mode:'edit',id:e.id,reservation_id:r.id,reservation_code:r.reservation_code,customer_name:r.customer_name,vendor_name:e.vendor_name||'',land_contract_id:e.land_contract_id||'',expense_type:e.expense_type||'land',remittance_stage:e.remittance_stage||'application',due_date:e.due_date||'',paid_date:e.paid_date||'',currency:e.currency||'KRW',foreign_amount:e.foreign_amount||'',exchange_rate:e.exchange_rate||'',amount_krw:e.amount_krw||'',note:e.note||''})
  }

  async function deleteRemittance(e){
    if(!has(member,'expense_manage'))return alert('지출·송금 삭제 권한이 없습니다.')
    if(!confirm('이 지출·송금 내역을 삭제하시겠습니까? 삭제 후에는 복구할 수 없습니다.'))return
    const {error}=await supabase.from('ops_expenses').delete().eq('organization_id',ORG).eq('id',e.id)
    if(error)return alert(error.message)
    await loadAll()
  }
  ...
  async function saveRemittance(){
    ...
    const payload={
      vendor_name:remitModal.vendor_name.trim(),expense_type:remitModal.expense_type||'land',
      ...
      amount_krw:krw,status:remitModal.paid_date?'paid':'pending',note:remitModal.note||null
    }
    const {error}=remitModal.mode==='edit'
      ?await supabase.from('ops_expenses').update(payload).eq('organization_id',ORG).eq('id',remitModal.id)
      :await supabase.from('ops_expenses').insert({...payload,organization_id:ORG,reservation_id:remitModal.reservation_id,created_by:session.user.id})
    if(error)return alert(error.message)
  ...
  return <div className="shell">
    ...
    <main>
      ...
      {page==='dashboard'&&<>
        <CaseDashboard
          organizationId={ORG}
          onOpenCase={(reservationId,tab)=>{const target=rows.find(row=>row.id===reservationId);if(target)openDetail(target,tab)}}
        />
        <section className="panel masterTodayWorkCenter">
          ...
          <div className="todayCenterStats"><div className={todayWorkSummary.overdue?'danger':''}><span>지연</span><b>{todayWorkSummary.overdue}</b></div><div><span>오늘</span><b>{todayWorkSummary.today}</b></div><div className={todayWorkSummary.due3?'warn':''}><span>D-3</span><b>{todayWorkSummary.due3}</b></div><div><span>D-7</span><b>{todayWorkSummary.due7}</b></div><div><span>잔금</span><b>{todayWorkSummary.balance}</b></div><div><span>최종체크</span><b>{todayWorkSummary.final}</b></div><div><span>여권</span><b>{todayWorkSummary.passport}</b></div><div><span>랜드사</span><b>{todayWorkSummary.land}</b></div><div><span>재확인</span><b>{todayWorkSummary.reconfirm}</b></div><div><span>설명회</span><b>{todayWorkSummary.briefing}</b></div><div><span>여행서류</span><b>{todayWorkSummary.docs}</b></div><div><span>출발안내</span><b>{todayWorkSummary.departure}</b></div></div>
          <div className="todayCenterFilters">{[['urgent','긴급·7일내'],['all','전체'],['customer_balance','잔금'],['final_check','최종체크'],['passport_copy','여권'],['reconfirm','재확인'],['briefing','설명회'],['travel_docs','여행서류'],['departure_notice','출발안내'],['intermediate_air_notice','중간항공'],['land_work','랜드사']].map(([k,l])=><button key={k} className={todayWorkFilter===k?'active':''} onClick={()=>setTodayWorkFilter(k)}>{l}</button>)}</div>
      ...
      {page==='staff'&&<section className="staffGrid">
        ...
        <div className="panel"><div className="panelHead"><div><h2>등록 직원</h2><p>권한 토글을 눌러 즉시 부여하거나 해제할 수 있습니다.</p></div><span className="badge">사용 중 {members.filter(m=>m.active).length}명</span></div>
          <div className="activeStaffSummary" aria-label="현재 사용 중인 직원">
            <strong>현재 사용 중</strong>
            <div className="activeStaffChips">{members.filter(m=>m.active).map(m=><span className="activeStaffChip" key={`active-${m.user_id}`} title={m.email||''}><b>{m.display_name||m.email}</b><em>{roleLabel[m.role]||m.role}</em></span>)}</div>
          </div>
    ...
    {remitModal&&<div className="modalBack remittanceBack"><div className={`modalBox remittanceModal ${isModalDirty('remittance',remitModal)?'hasUnsaved':''}`}><button className="close" onClick={()=>closeEditableModal('remittance',remitModal)}><X/></button><h2>{remitModal.mode==='edit'?'지출·송금 내역 수정':'랜드사 송금 등록'}</h2><p className="modalIntro">{remitModal.customer_name} · {remitModal.reservation_code}</p>
      ...
      </div><div className="modalActions"><button className="secondary" onClick={()=>closeEditableModal('remittance',remitModal)}>닫기</button><button className="primary" onClick={saveRemittance}><Save size={16}/> {remitModal.mode==='edit'?'수정 저장':'송금내역 저장'}</button></div>
    </div></div>}
