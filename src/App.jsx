...
export default function App(){
  ...
  return <div className="shell">
    ...
    {entityModal&&<div className="modalBack"><div className="modalBox operationalModal"><button className="close" onClick={()=>setEntityModal(null)}><X/></button><h2>{entityModal.mode==='edit'?'수정':'추가'} · {ENTITY_META[entityModal.type]?.label}</h2><p className="modalLead">{detailReservation?.customer_name} · {detailReservation?.reservation_code}</p>
      ...
      <div className="modalActions"><button className="secondary" onClick={()=>setEntityModal(null)}>취소</button><button className="primary" onClick={saveOperationalEntity}><Save size={16}/> 저장</button></div>
    </div></div>}
